...
def load_gesture_model():
    """Never call st.* inside a cached function — return status strings instead."""
    try:
...
def get_gesture_model():
    """Thin wrapper — calls st.* safely outside the cached function."""
...
class GestureProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if landmarks and self.model is not None:
            ...
            with self._lock:
                self._prediction_history.append(classID)
                if len(self._prediction_history) > 10:
                    self._prediction_history.pop(0)

                stable = (
                    self._prediction_history.count(classID) > 7
                    and confidence > 0.8
                )
                if stable:
...
def translate_to_telugu(text: str) -> str:
    """Translate any text to Telugu. Returns original on failure."""
    try:
...
def play_video_for_words(play_seq: list):
    ...
    # Cache hit — skip re-rendering if already combined
...
def main():
    ...
    # ----------------------------------------------------------
    # MODE 2 — Gesture → Text  (WebRTC live webcam)
    # ----------------------------------------------------------
    elif mode == "🤟 Gesture → Text (Live Webcam)":
        st.subheader("Live Gesture Recognition")
        st.caption("⚠️ Requires internet for WebRTC peer connection.")
        ...
        with col_info:
            ...
            if webrtc_ctx.video_processor:
                ...
                # Button registered once — never inside the polling loop
                ...
                while webrtc_ctx.state.playing:
                    ...
                    # Re-translate only when sentence changes
                    ...
                    if play_pressed and sentence:
                        play_video_for_words([w for w in sentence if w in list_of_words])
                        play_pressed = False
        ...
        if (not webrtc_ctx.state.playing
            ...
            if st.button("▶ Play ISL Video for Final Sentence"):
                play_video_for_words(
                    [w for w in st.session_state.gesture_sentence if w in list_of_words]
                )
